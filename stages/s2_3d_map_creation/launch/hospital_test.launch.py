...
def generate_launch_description():
    ...
    declare_world_file = DeclareLaunchArgument(
        'world_file',
        default_value=PathJoinSubstitution([pkg_dir, 'worlds', 'hospital.world']),
        description='Path to hospital world file'
    )
    ...
    rviz = Node(
        ...
        executable='rviz2',
        name='rviz2',
        arguments=['-d', PathJoinSubstitution([pkg_dir, 'config', 'hospital.rviz'])],
        parameters=[{'use_sim_time': use_sim_time}],
        output='screen'
    )
